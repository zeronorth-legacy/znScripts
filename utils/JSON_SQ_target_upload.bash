...
then
    echo "
Usage: `basename $0` <key file> <target file> <integration_name> <scenario_name> <suffix>
...
where,
  ...
  <target file>   - The SonarQube Project JSON files
...
input=$1
project=$(cat ${input} | jq -r '.components[] | .refKey+","+.name')
echo "$project" | while IFS="," read -r a b;
do	
	PROJECT_KEY="$a"
	TRGT_NAME="$b"
	TARGET_NAME=$(echo ${TRGT_NAME} | sed 's/&/and/g')
	if [ ! "PROJECT_KEY" ]
		then
		    echo "No Project keys provided! Exiting."
		    exit 1
	fi
...
POLICY_NAME="${TARGET_NAME}${POLICY_NAME_SUFFIX}"
POLICY_NAME_MOD=$(echo ${POLICY_NAME} | sed 's/ /%20/g')
TARGET_NAME_MOD=$(echo ${TARGET_NAME} | sed 's/ /%20/g' | sed 's/&/and/g')
