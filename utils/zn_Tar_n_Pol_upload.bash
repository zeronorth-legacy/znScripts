...
# Usage: `basename $0` -k API_KEY_FILE -K API_KEY -T TARGET_NAME -i INTEGRATION_ID  -t INTEGRATION_TYPE -p POLICY_TYPE -P POLICY_NAME -s SCENARIO_ID -d DATA_FILE -O OPCO_TAG -A APP_NAME_TAG -E ENV_TAG
...
while getopts ":k:K:T:i:I:p:P:S:d:O:E:A:v" opt; do
    case ${opt} in
        k )
            API_KEY_FILE="$OPTARG"
            ;;
...
if [ -n "$API_KEY_FILE" ]; then
    ...
    # print_msg "ZeroNorth Key: '$API_KEY'"
fi
...
# 2b) if the policy doesn't exist we create one
    ...
    if [ "$CUSTOM_PARM" ]; then
        ...
        print_msg "custom_parm is ${CUSTOM_PARM}"
    fi
    ...
    if [ "$POLICY_TYPE" == "manualUpload" ]; then
        ...
        pol_parms="{\"name\": \"${POLICY_NAME}\",\"environmentId\": \"${INTEGRATION_ID}\",\"environmentType\": \"${INTEGRATION_TYPE}\",\"targets\": [{\"id\": \"${tgt_id}\"}],\"policyType\": \"${POLICY_TYPE}\",\"scenarioIds\": [\"${SCENARIO_ID}\"],\"description\": \"${POLICY_DESCRIPTION}\"}"
    fi
