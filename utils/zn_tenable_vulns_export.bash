...
#
# Script for exporting Tenable.io Vulns for 1 plugin_id.
########################################################################
...
then
    echo "
Script for exporting Tenable.io Vulns for 1 plugin_id. Before using this
script, prepare a KEY/SECRET file as described below.


Usage: `basename $0` <key/secret file> <plugin_id> [<since>]
...
where,
  ...
  <plugin_id>  - This is tpyically a 5-digit number.
...
Examples:
  ...
  `basename $0` key.txt
  `basename $0` key.txt 12345
  `basename $0` key.txt 12345 60 > out.json
" >&2
    exit 1
fi
...
# Constants
########################################################################
TENABLE_URL="https://cloud.tenable.com/vulns/export"
CHUNK_SIZE=10000
...
if ( [ ! "$uuid" ] || [ "$uuid" == "null" ] ); then
    print_msg "ERROR: request for export failed:
$result"
    print_msg "Exiting."
    exit 1
fi
