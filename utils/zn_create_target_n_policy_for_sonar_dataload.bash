...
#set -xv
########################################################################
# (c) Copyright 2021, ZeroNorth, Inc., 2021-Apr, support@zeronorth.io
#
# Script to look-up/or create a Target and a SonarQube data load Policy
# with the specified Target and Policy names. If the named objects exist
...
then
    echo "
Script to look-up/or create a Target and a SonarQube data load Policy with the
specified Target and Policy names. If the named objects exist they are reused.
...
Usage: $MY_NAME <Project_Key> <Policy_Name> <ScenarioID> <IntegrationID> <Target_Name> [<key_file>]


where,

  <Project_Key>   - The SonarQube Project Key of the project whose results
                    to load.
...
Example: `basename $0` my-sonar-project My_Scan_Policy OtzOhx2SRxWhKjz10kqrnW XMopwSg7QqGglxtGAf9TSw My_Artifact_Target
" >&2
    exit 1
fi
...
########################################################################
result=$(curl -s -X GET  --header "${HEADER_CONTENT_TYPE}" --header "${HEADER_ACCEPT}" --header "${HEADER_AUTH}" "${URL_ROOT}/environments/${INTEGRATION_ID}")

# Check response code.
if [[ "$result" =~ '"statusCode":' ]]; then
    response=$(jq '.statusCode' <<< "$result")
    if [ $response -gt 299 ]; then
        print_msg "ERROR: API call to look up the Integration failed:\n${result}"
        exit 1
    fi
fi
...
tgt_id=$(find_by_name_twice targets "$TARGET_NAME") || exit 1
...
# None, so we create one.
    ...
    #echo -e "DEBUG: payload is:\n$data" >&2

    print_msg "Creating a target with name '${TARGET_NAME}'..."
    result=$(curl -s -X POST --header "${HEADER_CONTENT_TYPE}" --header "${HEADER_ACCEPT}" --header "${HEADER_AUTH}" -d "$data" "${URL_ROOT}/targets")

    # Check response code.
    if [[ "$result" =~ '"statusCode":' ]]; then
        response=$(jq '.statusCode' <<< "$result")
        if [ $response -gt 299 ]; then
            print_msg "ERROR: API call to create Target failed:\n${result}"
            exit 1
        fi
    fi
...
pol_id=$(find_by_name_twice policies "$POLICY_NAME") || exit 1
...
# None, we create one.
  ...
  \"permanentRunOptions\":
    ...
    if [[ "$result" =~ '"statusCode":' ]]; then
        response=$(jq '.statusCode' <<< "$result")
        if [ $response -gt 299 ]; then
            print_msg "ERROR: API call to create Policy failed:\n${result}"
            exit 1
        fi
    fi
